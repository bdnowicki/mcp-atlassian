...
- **One more Windows failure is intermittent, not constant**: `tests/unit/test_stdio_lifecycle.py::test_stdio_homebrew_probe_exits_after_stdin_close` spawns `uv run --frozen mcp-atlassian` and allows it 15s to exit. Measured on this host: it timed out twice while the `uv` cache was cold and several `uv`/`uvx` processes were contending for the cache lock, then passed three times in a row at 8–9s once the cache was warm. So a failure here is a statement about machine state, not about your change — re-run it alone before chasing it, and do not add it to the always-fails list above.
