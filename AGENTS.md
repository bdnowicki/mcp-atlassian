...
- **Windows/pytest**: five unit tests fail on Windows regardless of your change (POSIX file-permission bits, platform MIME/timestamp behaviour, and one subprocess-lifecycle probe that exceeds its 15s budget) — in `tests/unit/jira/test_attachments.py`, `tests/unit/utils/test_date.py`, `tests/unit/utils/test_media.py`, `tests/unit/utils/test_oauth.py`, `tests/unit/test_stdio_lifecycle.py::test_stdio_homebrew_probe_exits_after_stdin_close`. Compare against a clean worktree before attributing a failure to your work.
